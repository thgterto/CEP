const SPC = {
    ...
    computeIMR: (data) => {
        ...
        // Optimization: Single-pass O(N) loop with pre-allocated array instead of push() and spread syntax
        const mrData = new Array(len);
        mrData[0] = 0; // First element has no moving range
    ...
    computeXbarR: (data, n = 5) => {
        ...
        // Optimization: Pre-allocate arrays and calculate in a single O(N) pass,
        // avoiding slice(), map(), and spread operator (...g) overhead.
    ...
    computeXbarS: (data, n = 5) => {
        ...
        // Optimization: Single-pass over subgroups without slice() or map(),
        // preventing significant allocation/GC overhead.
        const numGroups = Math.floor(data.length / n);
        if (numGroups === 0) return { error: "Dados insuficientes para formar um subgrupo." };
        ...
        for (let i = 0; i < numGroups; i++) {
            ...
            // 1. Calculate subgroup mean inline
            for (let j = 0; j < n; j++) {
                sum += data[offset + j];
            }
            const mean = sum / n;
            ...
            // 2. Calculate subgroup stdDev inline
            let sumSq = 0;
            for (let j = 0; j < n; j++) {
