const SPC = {
    ...
    // --- Helpers ---
    // Optimized: Use for loop instead of reduce for better performance on large datasets
    mean: (arr) => {
        let sum = 0;
    ...
    stdDev: (arr, isSample = true, preCalculatedMean = null) => {
        ...
        let sumSq = 0;
        const len = arr.length;
        // Optimized: Use for loop instead of reduce and simple multiplication for squaring
        for (let i = 0; i < len; i++) {
            const diff = arr[i] - m;
            sumSq += diff * diff;
        }
