const SPC = {
    ...
    computeIMR: (data) => {
        ...
        let mrData;
        if (len === 0) {
            mrData = [0]; // Preserve exact baseline structure: [0, ...[]]
        } else if (len === 1) {
            mrData = [0];
            sumX = data[0];
        } else {
            mrData = new Array(len);
            mrData[0] = 0;
        ...
        const meanR = len > 1 ? sumR / rangesLen : NaN;
    ...
    // Optimization: Replacing array operations (push/shift) with pre-allocated arrays and extracting
    // constant loop-invariant operations to improve performance by over 2x.
    ...
    // Optimization: Cache jStat calculations (e.g. j.quartiles(), j.mean(), j.stdev(true))
    // to prevent redundant iterations and internal sorts, yielding ~2x execution speedup.
    ...
    computeCapability: (data, usl, lsl, sigmaST) => {
        ...
        // Optimization: pass mu to stdDev to avoid redundant mean calculation array iterations
