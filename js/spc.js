const SPC = {
    ...
    computeXbarR: (data, n = 5) => {
        ...
        // Optimization: Calculate subgroups directly without array allocation and .map()
        const numSubgroups = Math.floor(data.length / n);
        const xbars = new Array(numSubgroups);
        const ranges = new Array(numSubgroups);
        ...
        const xdbar = numSubgroups > 0 ? xbarsSum / numSubgroups : 0;
        const rbar = numSubgroups > 0 ? rangesSum / numSubgroups : 0;
    ...
    computeXbarS: (data, n = 5) => {
        ...
        const numSubgroups = Math.floor(data.length / n);
        const xbars = new Array(numSubgroups);
        const sigmas = new Array(numSubgroups);
        ...
        for (let i = 0; i < numSubgroups; i++) {
            ...
            // isSample = true means we divide by (n - 1)
        ...
        const xdbar = numSubgroups > 0 ? xbarsSum / numSubgroups : 0;
        const sbar = numSubgroups > 0 ? sigmasSum / numSubgroups : 0;
