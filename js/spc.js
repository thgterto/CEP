const SPC = {
    ...
    computeXbarR: (data, n = 5) => {
        ...
        // Optimization: Avoid data.slice(), .map(), and spread operator (...g)
        // by pre-allocating arrays and processing in a single pass.
    ...
    computeXbarS: (data, n = 5) => {
        ...
        // Optimization: Avoid data.slice() and .map() by pre-allocating
        // arrays and computing mean/stdDev inline.
        const numGroups = Math.floor(data.length / n);
        const xbars = new Array(numGroups);
        ...
        for (let i = 0; i < numGroups; i++) {
            ...
            // Standard deviation for sample (n-1)
