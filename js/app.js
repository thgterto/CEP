const App = {
    ...
    loadDemo: () => {
        const data = Utils.generateDemoData();
        App.state.data = data;
    ...
    loadPaste: () => {
        ...
        const data = Utils.parsePaste(text);
        if (data.length < 2) {
    ...
    exportCSV: (type) => {
        if (type === 'violations') {
             ...
             Utils.downloadCSV(App.state.data, 'cep_dados.csv');
        }
    },

    exportXLSX: () => {
        if (!App.state.data.length) return;
        Utils.exportXLSX(App.state.data, App.state.violations, App.state.notes, 'cep_analise.xlsx');
    }
};
