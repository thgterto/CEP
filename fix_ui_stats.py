...
# Regex to find renderStats function block.
# It starts with "renderStats: (stats) => {" and ends with "}," before "renderCapability"
# I'll use a broad match again.
pattern = r'renderStats: \(stats\) => \{[\s\S]*?\},(?=\s*renderCapability)'
match = re.search(pattern, content)

if match:
    # We replace the match with new_render_stats + "," to maintain syntax
    new_content = content.replace(match.group(0), new_render_stats.strip() + ",")
    with open('js/ui.js', 'w') as f:
        f.write(new_content)
    print("Fixed renderStats")
else:
    print("Could not find renderStats function to fix")
